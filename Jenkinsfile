  ...
  agent {label 'main'}
    stages{
       stage('hosting application'){
        steps{
          sh "ls"
          sh "aws rds create-db-instance --db-instance-identifier test-mysql-instance3 --db-name cpms --db-instance-class db.t2.micro --vpc-security-group-ids sg-052b7d954440d8def --engine mysql --engine-version 5.7 --db-parameter-group-name default.mysql5.7 --publicly-accessible  --master-username admin --master-user-password arvind123 --allocated-storage 10 --region us-east-1"
          sleep(450)
          script{
          ...
          script{
              def cmd = "aws elbv2 create-load-balancer --name my-load-balancer --subnets subnet-09655f734e9b8cbcc subnet-00817cbb350bf4216 --security-groups sg-052b7d954440d8def --region us-east-1 "
              def output = sh(script: cmd,returnStdout: true)
          ...
          script{
              def cmd = "aws elbv2 create-target-group --name my-targets --protocol HTTP --port 80 --target-type instance --vpc-id vpc-015c5ca6f2cb95686 --region us-east-1"
              def output = sh(script: cmd,returnStdout: true)
           ...
           sh "aws autoscaling create-launch-configuration --launch-configuration-name my-lc3-cli --image-id ami-04505e74c0741db8d --instance-type t2.micro --security-groups sg-052b7d954440d8def --key-name sshkey1 --iam-instance-profile demo --user-data file://userdata.txt --region us-east-1"
